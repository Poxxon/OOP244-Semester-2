...
int main() {
    ...
    pd.set(12345);
    if (pd.onLoan()) {
        cout << "Now this publication is on loan to a member with the id: 12345"
            << endl;
        pd.resetDate();
    ...
    cout << "Adding the periodical publication to the end of the data file:"
        << endl;
    ...
    for (int row = 1; filein; row++) {
        filein >> pType;
        if (pType != 'P') {
            cout << "The Record type signature is supposed to be B, but it is: "
                << pType << endl;
